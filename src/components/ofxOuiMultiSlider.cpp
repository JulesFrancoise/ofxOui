...
void ofxOui::MultiSlider::mousePressed(ofMouseEventArgs &e) {
    ...
    if (inside(e.x, e.y) && e.button == 0) {
        ...
        if (slider_rect_.inside(e.x, e.y)) {
            ...
            clipValues();
            ofxOui::MultiSlider::ValueEvent event =
                ofxOui::MultiSlider::ValueEvent(this, slider_index,
                                                   values[slider_index]);
...
void ofxOui::MultiSlider::mouseDragged(ofMouseEventArgs &e) {
    if (disabled || (blocking_component && blocking_component != this)) return;
    ...
    if (layout == Layout::Vertical) {
        ...
        if (previous_slider_index == slider_index) {
            ...
            for (int i = previous_slider_index + 1; i < slider_index + 1; i++) {
                ...
                if (multislider_event_callback_ != nullptr) {
                    ofxOui::MultiSlider::ValueEvent event =
                        ofxOui::MultiSlider::ValueEvent(this, i, values[i]);
                    multislider_event_callback_(event);
                }
            }
        } else {
...
ofxOui::MultiSlider::ValueEvent::ValueEvent(MultiSlider *sender_, int index_,
                                               float value_)
